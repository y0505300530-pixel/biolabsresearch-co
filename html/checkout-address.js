/*! checkout-address.js v3 — browser autofill + Photon suggestions + optional Google Places */
(function () {
  'use strict';
  ...
  function boot() {
    ...
    function parsePhoton(feature) {
      ...
      var labelBits = [street || p.name, p.city || p.town || p.village, p.state, p.postcode, p.countrycode]
        .filter(Boolean);
      return {
    ...
    function searchPhoton(q) {
      var url =
        ...
        '&limit=6&lang=en&lat=40.7&lon=-74&zoom=6';
      fetch(url, { headers: { Accept: 'application/json' } })
        ...
        .then(function (data) {
          ...
          openDd(
            feats.map(parsePhoton).filter(function (x) {
              return x.parts.line1 || x.label;
            })
          );
        })
        .catch(function () {
    ...
    function initGoogle() {
      ...
      if (hint) {
        hint.textContent =
          'Start typing your street — Google address suggestions fill city, state, and postal code.';
      }
      closeDd();
      ...
      ac.addListener('place_changed', function () {
        ...
        fillFromComponents({
          line1:
            [streetNum, route].filter(Boolean).join(' ').trim() || place.name || '',
          city:
          ...
          state: map.administrative_area_level_1
            ? map.administrative_area_level_1.short_name
            : '',
    ...
    if (KEY) {
      ...
      s.src =
        ...
        '&libraries=places&callback=__blrPlacesReady';
      s.async = true;
      s.defer = true;
      document.head.appendChild(s);
    } else if (hint) {
      hint.textContent =
        'Type your street for instant suggestions — city, state, and postal code fill in. Browser autofill also works.';
    }
  }
