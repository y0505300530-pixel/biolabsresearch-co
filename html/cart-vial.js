...
function productUrl(item){
  ...
  if (!slug) return '/index.html#catalog';
  return '/products/' + slug + '.html';
}
function vialImg(item){
  return '/media/vial-' + productSlug(item) + '.webp?v=91';
}
var CART_SUGGEST = [
  ...
  {slug:'retatrutide', name:'R3TA', price:195}
];
function addMoreHtml(cart){
  ...
  list.forEach(function(p){
    html += '<div class="cart-addcard">' +
      '<a href="/products/' + p.slug + '.html"><img src="/media/vial-' + p.slug + '.webp?v=92" alt="' + p.name + '" width="110" height="96"></a>' +
...
function addSuggest(slug, name, price){
  if (typeof getCart === 'function') {
    ...
    if (ex) ex.qty += 1;
    else c.push({name:name, price:price, qty:1, slug:slug, imageUrl:'/media/vial-'+slug+'.webp?v=92'});
  ...
  if (typeof cart !== 'undefined') {
    ...
    if (ex2) ex2.qty += 1;
    else cart.push({name:name, price:price, qty:1, slug:slug, imageUrl:'/media/vial-'+slug+'.webp?v=92'});
