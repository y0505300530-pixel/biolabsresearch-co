  ...
  window.pdpStoryHtml = function(p){
    ...
    var also = rel.map(function(s){
      ...
      return '<a class="pdp-also-card" href="/products/'+s+'">'
        + '<img src="/media/vial-'+s+'.webp?v=181" alt="'+NAMES[s]+'">'
    ...
    return ''
      ...
      + '<article><div class="pdp-doc-art">'+figCoa+'</div><h3>Certificate of Analysis</h3><p>Lot number, date, laboratory, and method for this '+name+' vial sit on the matching PDF when a file is hosted. Otherwise use <strong>Request lot COA</strong>.</p></article>'
      + '</div></div></section>'
      ;
  };
})();
