...
|---|---|---|---|
| P1 | Card checkout POSTs to the live CRM sidecar (`https://crm.biolabsresearch.co/api/checkout/charge`, camelCase `idempotencyKey`). Visible tip stays v2.99a until Soft-QA authorize — do not stamp 2.99b yet. Infra review before customers see live charges. Soft-QA: sandbox authorize + decline; confirm no PAN/CVV in console or localStorage; GA `checkout_complete` only on `ok===true`. | infra | open |
