...
| P8 | v2.99g COA: emit `coa_pdf_url` (required when a lot PDF exists) and optional `coa_lot` on `/api/products`. Storefront already reads them (`html/coa-pdp.js`). See `docs/CRM-COA-API.md`. Until the fields ship, PDP stays on Request lot COA. | infra | open |
