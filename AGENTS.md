...
Visible site tip is **v2.99g1** (`catalog-filter-flicker-fix`) on top of **v2.99g** (`hub-spoke-dynamic-coa`) / **v2.99f1** (`catalog-mobile-2col-compact`) /
...
**G3-R (v2.99e1, kept under g):** Public catalog name is **G3-R** only (`/products/g3-r`; gold-band **G3-R**). Old retatrutide/r3ta/reta paths 301. Do not resurrect `html/products/retatrutide.html`. Soft-QA Gate 0 stamp for this tip is **v2.99g1**.
...
**IndexNow (same key as d):** `html/indexnow-key.txt` and `html/{key}.txt`. **v2.99g1 deploy must run IndexNow after live** — `scripts/indexnow-ping.sh` (or the curl in `VERSION.md`). Do not block abandon capture on this ping.
